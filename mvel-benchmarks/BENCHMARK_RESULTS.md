...
- Current: post-error-handling + DRL cleanup (commit a374e48)
...
| constructAndEvalPojo (thrpt) | 47.99 ops/us | 43.41 ops/us | -10% |
...
### constructAndEvalPojo Regression (-10%)

Context construction alone is flat at 0.010 us, but the combined construct+eval is 0.023 us — something is allocating or competing between those two operations that wasn't before. Likely the exception hierarchy changes added allocation on some path that runs during the construct+eval cycle. Worth profiling with async-profiler before ASM work begins.
